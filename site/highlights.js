(function(){
    ...
    proseEls().forEach(function(prose){
      ...
      Object.keys(byCidx).forEach(function(ci){
        ...
        byCidx[ci].sort(function(a,b){ return a.start-b.start; }).forEach(function(h){
          ...
          var first=p.querySelector('mark.hl[data-hid="'+h.id+'"]');
          if(first && !first.id){ first.id='hl-'+h.id; first.title='Highlighted - click to remove'; }
        });
      });
    });
  }
  ...
  function showPop(mark){
    ...
    var b=document.createElement('button'); b.type='button'; b.className='hl-pop-remove';
    b.textContent='Remove highlight';
    b.addEventListener('click', function(e){ e.stopPropagation(); removeHl(id); hidePop(); });
    pop.appendChild(b);
