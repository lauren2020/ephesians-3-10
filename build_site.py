...
# is keyed by chapter + paragraph index (data-chap / data-cidx), matching the anchors
# used by the scroll view, so a result can scroll straight to its paragraph.
search_index = {
    "chapters": {str(n): t for n, t, _ in chapter_data},
    "paras": [{"c": n, "i": i, "x": p}
              for n, t, ps in chapter_data for i, p in enumerate(ps)],
}
search_index["chapters"]["0"] = "Dedication"
search_index["paras"] += [{"c": 0, "i": i, "x": p} for i, p in enumerate(ded)]
...
def topbar(show_contents=True, view="scroll"):
    ...
    search = ('<div class="bar-center">'
              '<input type="search" class="book-search-input" placeholder="Search the book…" '
              'aria-label="Search the book" autocomplete="off" spellcheck="false">'
              '<div class="book-search-results" role="listbox" hidden></div>'
              '</div>')
